...
3. **Resume detection:** Check for existing files matching `{planning_artifacts}/prfaq-*.md`.
   - If none exist: continue with a fresh session.
   - If exactly one exists: read only its first 20 lines to extract the frontmatter `stage` field, set `prfaq_file` to that path, derive `prfaq_distillate_file` by replacing `.md` with `-distillate.md`, and offer to resume from the next stage.
   - If multiple exist: present the matching filenames, ask the user which concept to resume, then read only the first 20 lines of the selected file to extract `stage`, set `prfaq_file` to that path, and derive `prfaq_distillate_file` by replacing `.md` with `-distillate.md`.
   - Do not read full PRFAQ documents during resume detection.

4. **Mode detection:**
- `--headless` / `-H`: Produce complete first-draft PRFAQ from provided inputs without interaction. Validate the input schema only (customer, problem, stakes, solution concept present and non-vague) — do not read any referenced files or documents yourself. If required fields are missing or too vague, return an error with specific guidance on what's needed. Derive `concept_slug` from the provided concept name or a short customer-problem solution label, set `prfaq_file = {planning_artifacts}/prfaq-{concept_slug}.md`, set `prfaq_distillate_file = {planning_artifacts}/prfaq-{concept_slug}-distillate.md`, fan out artifact analyzer and web researcher subagents in parallel (see Contextual Gathering below) to process all referenced materials, then create the output document at `{prfaq_file}` using `./assets/prfaq-template.md` and route to `./references/press-release.md`.
...
**Create the output document** at `{prfaq_file}` using `./assets/prfaq-template.md`. If `prfaq_file` is not already set from resume detection, derive `concept_slug` from a short confirmed concept title or customer-problem label, set `prfaq_file = {planning_artifacts}/prfaq-{concept_slug}.md`, and set `prfaq_distillate_file = {planning_artifacts}/prfaq-{concept_slug}-distillate.md`. If `prfaq_file` was set during resume detection and `prfaq_distillate_file` is still unset, derive `prfaq_distillate_file` by replacing `.md` with `-distillate.md` on `prfaq_file`. Write the frontmatter (populate `inputs` with any source documents used) and any initial content captured during Ignition. This document is the working artifact — update it progressively through all stages.
