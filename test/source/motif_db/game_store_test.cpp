...
// llvm-prefer-static-over-anonymous-namespace and misc-use-anonymous-namespace are mutually
// contradictory; anonymous namespace is preferred by the C++ standard.
...
struct db_fixture {
    ...
    // Count rows in any table — used to verify deduplication and cascade behaviour.
...
[[nodiscard]] auto make_player(std::string name) -> motif::db::player { // NOLINT(llvm-prefer-static-over-anonymous-namespace)
...
[[nodiscard]] auto make_game_with_event(std::string event_name) -> motif::db::game { // NOLINT(llvm-prefer-static-over-anonymous-namespace)
...
TEST_CASE("game_store: same player name is reused across games", "[motif_db][game_store]") {
    ...
    // "Magnus" appears as white in both games; should produce a single player row.
