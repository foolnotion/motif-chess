...
struct duck_fixture {
    ...
    ~duck_fixture()
    {
        ...
        if (duck_db != nullptr) { duckdb_close(&duck_db);      }
    }
};
          ...
          "[motif-db][position_store]")
{
    ...
    std::vector<motif::db::position_row> rows{
        {.zobrist_hash = hash_a, .game_id = 1, .ply = 1, .result = 1,
         .white_elo = {elo_white}, .black_elo = {elo_black}},
        {.zobrist_hash = hash_b, .game_id = 1, .ply = 2, .result = 1,
         .white_elo = {elo_white}, .black_elo = {elo_black}},
    };
          ...
          "[motif-db][position_store]")
{
    ...
    std::vector<motif::db::position_row> rows{
        {.zobrist_hash = hash_null_elo, .game_id = 2, .ply = 1, .result = 0,
         .white_elo = {}, .black_elo = {}},
    };
          ...
          "[motif-db][position_store]")
{
    ...
    REQUIRE(duckdb_query(fix.con,
        "SELECT zobrist_hash, game_id, ply, result, white_elo, black_elo "
        "FROM position",
        &qres) == DuckDBSuccess);
