...
#### Second Review Findings (2026-04-24)

- [x] [Review][Patch] Transposition null subtree — `built_nodes` entry not erased after `std::move`; second parent whose continuation resolves to the same child key receives null `subtree`, causing UB on dereference. Fix: erase the entry after moving so a second consumer falls through to the `build_continuation` fallback leaf node. [`source/motif/search/opening_tree.cpp:402-404`]
- [x] [Review][Patch] SQLite unbounded `IN` clause — previous 500-item batch removed with no replacement; `get_game_contexts` now builds a single `IN (?, ?, …)` with one placeholder per game ID, failing with `SQLITE_ERROR` ("too many SQL variables") for popular positions in large corpora. Restored 500-item batching. [`source/motif/db/game_store.cpp`]
- [x] [Review][Patch] Silent `uint16_t` overflow in ply arithmetic — `occurrence.root_ply + row.depth` and `sample_root_ply + nkey.depth - 1U` are computed via `int` promotion then cast to `uint16_t`; wraps silently if sum > 65535. Fixed with `std::size_t` arithmetic and overflow guard before cast. [`source/motif/search/opening_tree.cpp:340,388`]
- [x] [Review][Patch] No test for `prefetch_depth=1` — the degenerate case where every continuation is a boundary node is untested; the `build_continuation`/`built_nodes` attachment branch can never fire at depth 1, leaving that code path unverified. Added test case. [`test/source/motif_search/opening_tree_test.cpp`]
- [x] [Review][Patch] Orphaned `game_id` returns `io_failure` instead of skipping — when a game context is not found in the aggregation phase or the build phase, the code returns `tl::unexpected{error_code::io_failure}`. Changed to `continue`. [`source/motif/search/opening_tree.cpp:295-297,384-386`]
- [x] [Review][Patch] ECO/`opening_name` fields untested — AC1 requires ECO code and opening name per continuation; every test inserts games with `eco` and `opening_name` but no test asserts the resulting `continuation.eco` or `continuation.opening_name` values. Assertions added to depth-1 test. [`test/source/motif_search/opening_tree_test.cpp`]
- [x] [Review][Patch] Average Elo fields untested — AC1 requires `average_white_elo`/`average_black_elo` per continuation; tests insert games with Elo values but no test asserts the computed averages. Assertions added with `WithinAbs` matcher. [`test/source/motif_search/opening_tree_test.cpp`]
- [x] [Review][Patch] `io_failure` on empty `built_nodes` after all orphaned games are skipped — if every occurrence is skipped, `built_nodes` has no depth-1 entry and the final lookup returned `io_failure`; now returns an empty expanded tree instead. [`source/motif/search/opening_tree.cpp:427`]
- [x] [Review][Defer] Quadratic `replay_position` calls — `O(occurrences × max_depth × avg_root_ply)` move applications with no incremental caching across depths. Performance concern, not a correctness issue. [`source/motif/search/opening_tree.cpp:300,340,389`] — deferred, pre-existing
- [x] [Review][Defer] Zobrist collision merges different positions into the same node aggregate — pre-existing architecture risk inherent to Zobrist hashing. [`source/motif/search/opening_tree.cpp:313`] — deferred, pre-existing
- [x] [Review][Defer] `const_cast` delegation in `game_store::get_game_contexts` non-const overload — safe but redundant non-const overload; pre-existing pattern. [`source/motif/db/game_store.cpp`] — deferred, pre-existing
