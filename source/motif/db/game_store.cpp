...
// NOLINT(llvm-prefer-static-over-anonymous-namespace): conflicts with misc-use-anonymous-namespace
[[nodiscard]] auto prepare(sqlite3* conn, char const* sql) -> result<unique_stmt> { // NOLINT(llvm-prefer-static-over-anonymous-namespace)
...
// sqlite3_column_text returns const unsigned char*; adapts it to char const* safely.
[[nodiscard]] auto column_text(sqlite3_stmt* stmt, int col) -> char const* { // NOLINT(llvm-prefer-static-over-anonymous-namespace)
...
auto bind_optional_text( // NOLINT(llvm-prefer-static-over-anonymous-namespace)
    sqlite3_stmt* stmt, int col, std::optional<std::string> const& val) -> void
{
    if (val) {
...
auto bind_optional_int( // NOLINT(llvm-prefer-static-over-anonymous-namespace)
    sqlite3_stmt* stmt, int col, std::optional<std::int32_t> const& val) -> void
{
    if (val) {
...
[[nodiscard]] auto column_optional_text( // NOLINT(llvm-prefer-static-over-anonymous-namespace)
    sqlite3_stmt* stmt, int col) -> std::optional<std::string>
{
...
[[nodiscard]] auto column_optional_int32( // NOLINT(llvm-prefer-static-over-anonymous-namespace)
    sqlite3_stmt* stmt, int col) -> std::optional<std::int32_t>
{
    if (sqlite3_column_type(stmt, col) == SQLITE_NULL) {
        return std::nullopt;
    }
...
[[nodiscard]] auto exec(sqlite3* conn, char const* sql) -> result<void> { // NOLINT(llvm-prefer-static-over-anonymous-namespace)
...
[[nodiscard]] auto foreign_keys_enabled(sqlite3* conn) -> result<bool> { // NOLINT(llvm-prefer-static-over-anonymous-namespace)
...
auto game_store::get(std::uint32_t game_id) -> result<game> {
    // language=sql
    static constexpr char const* sql = R"sql(
        SELECT
