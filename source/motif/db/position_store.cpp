...
auto position_store::query_opening_stats(std::uint64_t const zobrist_hash) const -> result<std::vector<opening_stat_agg_row>>
{
    ...
    sql << "WITH deduped AS ("
        ...
        << "GROUP BY p_root.game_id, p_cont.encoded_move, p_cont.zobrist_hash, "
           ...
           "SELECT "
           "uniq.zobrist_hash, "
           "COUNT(DISTINCT uniq.game_id) AS frequency, "
           "COUNT(DISTINCT CASE WHEN uniq.result > 0 THEN uniq.game_id END) AS white_wins, "
           "COUNT(DISTINCT CASE WHEN uniq.result = 0 THEN uniq.game_id END) AS draws, "
           "COUNT(DISTINCT CASE WHEN uniq.result < 0 THEN uniq.game_id END) AS black_wins, "
           ...
           "FROM position "
           "WHERE zobrist_hash IN (SELECT DISTINCT child_hash FROM deduped)"
           ") AS uniq "
