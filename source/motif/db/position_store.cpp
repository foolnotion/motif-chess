...
auto position_store::query_by_zobrist(std::uint64_t const zobrist_hash) const -> result<std::vector<position_match>>
{
    duckdb_result res {};
    ...
    -> result<std::vector<tree_position_row>>
{
    ...
    for (std::size_t i = 0; i < row_count; ++i) {
        ...
        if (!duckdb_value_is_null(&res, white_elo_col, row_idx)) {
            white_elo = static_cast<std::int16_t>(duckdb_value_int16(&res, white_elo_col, row_idx));
        }
        auto black_elo = std::optional<std::int16_t> {};
        if (!duckdb_value_is_null(&res, black_elo_col, row_idx)) {
            black_elo = static_cast<std::int16_t>(duckdb_value_int16(&res, black_elo_col, row_idx));
        }
