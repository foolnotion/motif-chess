...
void register_routes(httplib::Server& svr, motif::db::database_manager const& dbmgr)
{
    svr.Get("/health",
            [](httplib::Request const& /*req*/, httplib::Response& res) -> void
            {
    ...
    svr.Get("/api/positions/:zobrist_hash",
            [&dbmgr](httplib::Request const& req, httplib::Response& res) -> void
            {
                ...
                auto matches = motif::search::position_search::find(dbmgr, hash_val, limit, offset);
                if (!matches) {
